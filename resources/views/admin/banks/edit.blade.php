          ...
          <div class="card z-index-0 fadeIn3 fadeInBottom">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
              <div class="bg-gradient-primary shadow-primary border-radius-lg py-2 pe-1">
                <h4 class="text-white font-weight-bolder text-center mb-2">Bank Edit</h4>
              </div>
            </div>
            <div class="card-body">
              <form role="form" class="text-start" action="{{ route('admin.banks.update', $bank->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="custom-form-group">
                  ...
                  @enderror
                  <img src="{{ $bank->img_url }}" width="100px" class="rounded" alt="">
                </div>

                <div class="custom-form-group">
                  <label for="bank">Bank Name</label>
                  <input type="text" id="bank" class="form-control" name="bank" placeholder="Enter Bank Name" value="{{ $bank->bank }}">
                  @error('bank')
                    <span class="text-danger">*{{ $message }}</span>
                  @enderror
                </div>
                <div class="custom-form-group">
                  ...
                  <input type="text" id="username" class="form-control" name="name" placeholder="Enter Username" value="{{ $bank->name }}">
                  @error('name')
                    <span class="text-danger">*{{ $message }}</span>
                  @enderror
                </div>
                <div class="custom-form-group">
                  ...
                  <input type="number" id="phone" class="form-control" name="phone" placeholder="Enter Phone" value="{{ $bank->phone }}">
                  @error('phone')
                    <span class="text-danger">*{{ $message }}</span>
                  @enderror
                </div>
                <div class="mb-3 d-flex">
                  ...
                  <div>
                    <label for="baht" class="form-label">
                      <input type="radio" {{ $bank->currency == "baht" ? "checked" : "" }} name="currency"  id="baht" value="baht">
                      ဘတ်
                    </label>
                  </div>
                </div>
