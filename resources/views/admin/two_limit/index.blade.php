        ...
        <div class="modal-dialog mt-lg-10">
         <div class="modal-content">
          <div class="modal-header">
           <h5 class="modal-title" id="ModalLabel">Create New TwoDLimit</h5>
           <i class="material-icons ms-3">file_upload</i>
           <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
           <p>You can create new TwoDLimit or all users.</p>
           <form class="multisteps-form__form" action="{{ route('admin.two-digit-limit.store') }}" method="post">
            @csrf
            <div class="input-group input-group-dynamic mb-3">
             <label class="form-label">TwoDLimit </label>
             <input type="text" class="form-control" name="two_d_limit" onfocus="focused(this)" onfocusout="defocused(this)">
            </div>
            <div class="modal-footer">
             <button type="button" class="btn bg-gradient-secondary btn-sm" data-bs-dismiss="modal">Close</button>
             <button type="submit" class="btn bg-gradient-primary btn-sm">Save TwoDLimit<button>
            </div>
           </form>
     ...
     <tbody>
      ...
      <tr>
       ...
       <td>
        ...
        <form class="d-inline" action="{{ route('admin.two-digit-limit.destroy', $limit->id) }}" method="POST">
         @csrf
         @method('DELETE')
