...
class WinnerPrizeCheck implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        // Process winning entries for prize_one
        $this->processWinningEntries((string) $this->prize->prize_one, 'greater');

        // Process winning entries for prize_two
        $this->processWinningEntries((string) $this->prize->prize_two, 'less');
    }
    ...
    protected function processWinningEntries($prizeNumber, $comparison)
    {
        $today = Carbon::today();

        $winningEntries = DB::table('lotto_three_digit_copy')
            ->join('lottos', 'lotto_three_digit_copy.lotto_id', '=', 'lottos.id')
            ->where(function ($query) use ($prizeNumber, $comparison) {
                if ($comparison === 'greater') {
                    $query->where('lotto_three_digit_copy.bet_digit', '>', $prizeNumber);
                } else { // 'less'
                    $query->where('lotto_three_digit_copy.bet_digit', '<', $prizeNumber);
                }
            })
        ...
        foreach ($winningEntries as $entry) {
            DB::transaction(function () use ($entry) {
                ...
                $user = $lottery->user;
                $user->balance += $entry->sub_amount * 10; // Update this based on your prize calculation
                $user->save();
